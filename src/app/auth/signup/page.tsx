...
export default function SignupPage() {
  ...
  const handleSignup = async (e: React.FormEvent) => {
    ...
    else { toast.success("Check your email to confirm your account."); router.push("/auth/login"); }
    setLoading(false);
  };

  const inputClass = "w-full pl-10 pr-4 py-3 bg-white/10 border border-white/20 rounded-xl text-white placeholder:text-white/30 focus:outline-none focus:ring-2 focus:ring-blue-400/50 focus:border-blue-400/50 transition-all";
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-blue-950 to-indigo-950 flex items-center justify-center px-4 py-12">
      ...
      <div className="relative w-full max-w-md animate-scale-in">
        <div className="text-center mb-8">
          ...
          <p className="text-blue-300/70 mt-1 text-sm">Join FlightX and start booking</p>
        </div>
        ...
        <div className="bg-white/10 backdrop-blur-xl border border-white/20 rounded-3xl p-8 shadow-2xl">
          <form onSubmit={handleSignup} className="space-y-4">
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input type={showPassword ? "text" : "password"} value={password} onChange={(e) => setPassword(e.target.value)} className="w-full pl-10 pr-12 py-3 bg-white/10 border border-white/20 rounded-xl text-white placeholder:text-white/30 focus:outline-none focus:ring-2 focus:ring-blue-400/50 focus:border-blue-400/50 transition-all" placeholder="Min. 6 characters" required />
            ...
            <div>
              ...
              <div className="relative">
                <Lock className="absolute left-3.5 top-1/2 -translate-y-1/2 w-4 h-4 text-white/40" />
                <input type={showPassword ? "text" : "password"} value={confirmPassword} onChange={(e) => setConfirmPassword(e.target.value)} className={inputClass} placeholder="Repeat password" required />
              </div>
            </div>
          ...
          <div className="mt-5 text-center">
            <p className="text-sm text-white/50">
              ...
              <Link href="/auth/login" className="text-blue-300 font-medium hover:text-blue-200 transition-colors">Sign in</Link>
            </p>
          </div>
        </div>
      </div>
